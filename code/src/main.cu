        ...
        for (int j = 0; j < num_batches; j++) {
            ...
            for (int s = batch_start; s < batch_end; s++) {
                ...
                for (int h = 0; h < hidden_layer_size; h++) {
                    ...
                    thrust::copy(d_W1.begin() + W_row_h, d_W1.begin() + W_row_h + num_parameters, d_temp_W_row.begin());
                    thrust::copy(d_X.begin() + (s * num_parameters), d_X.begin() + (s * num_parameters) + num_parameters, d_temp_X_row.begin());
        
                    d_hidden[h] = thrustnn::dot(d_temp_W_row, d_temp_X_row) + d_b1[h];
                }
                ...
                for (int c = 0; c < num_classes; c++) {
                    ...
                    thrust::copy(d_W2.begin() + W_row_c, d_W2.begin() + W_row_c + hidden_layer_size, d_temp_W_row.begin());
                    thrust::copy(d_hidden.begin(), d_hidden.begin() + hidden_layer_size, d_temp_X_row.begin());
        
                    d_logits[c] = thrustnn::dot(d_temp_W_row, d_temp_X_row) + d_b2[c];
                }
